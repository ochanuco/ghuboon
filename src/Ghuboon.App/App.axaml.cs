...
public partial class App : Application
{
    ...
    private MainWindowViewModel BuildMainViewModelWithRealDependencies()
    {
        ...
        // 6a. OS desktop notifications.
        WireDesktopNotifications(_syncService, dbFactory, clock);
        ...
        var itemCtxFactory = BuildItemContextFactory(
            notificationRepo, eventRepo, apiClient, clock, appSettings,
            credentialStore, bookmarks, threading, () => mainVmRef);
    ...
    /// <summary>
    /// Build the production <see cref="GitHubApiClient"/> and the
    /// underlying <see cref="HttpClient"/>. Assigns <c>_httpClient</c>
    /// as a side effect because the app lifecycle owns disposal.
    /// <para>
    /// SocketsHttpHandler-tuned for laptop sleep/wake survival. The
    /// previous setup (default handler + <c>HttpClient.Timeout=15s</c>)
    /// wedged after macOS sleep on 2026-05-14: a half-dead TCP
    /// connection in the default pool stayed pooled across sleep, and a
    /// subsequent <c>SendAsync</c> never returned because the
    /// wall-clock-frozen sleep period broke the internal
    /// <c>CancelAfter</c> timer used by <c>HttpClient.Timeout</c>.
    /// Result: SyncGate held, every later <c>sync.SyncAsync</c> queued
    /// behind <c>WaitAsync</c> forever.
    /// </para>
    /// <para>
    /// Three knobs handle the regression:
    ///   * <c>PooledConnectionLifetime = 3 min</c> — even an apparently-
    ///     healthy connection is rotated out, so a stale one cannot be
    ///     reused indefinitely.
    ///   * <c>PooledConnectionIdleTimeout = 1 min</c> — idle connections
    ///     dropped before macOS's TCP keepalive would normally notice.
    ///   * <c>ConnectTimeout = 10 s</c> — fail fast on TCP connect rather
    ///     than waiting on the OS-level SYN retransmit.
    /// We still set <c>HttpClient.Timeout=15s</c> as a per-request
    /// safety net (now backed by <c>SocketsHttpHandler</c>'s own
    /// deadline path).
    /// </para>
    /// </summary>
    private GitHubApiClient CreateApiClient()
    {
        ...
        return new GitHubApiClient(_httpClient, _logger!);
    }

    /// <summary>
    /// Subscribe to <see cref="NotificationSyncService.NewNotifications"/>
    /// and dispatch OS desktop banners. The gate dedupes via
    /// <c>notification_local_states.last_notified_at</c>; the platform
    /// service shells out to <c>osascript</c> on macOS. Banner spawns
    /// fire in parallel because each <c>osascript</c> spawn is
    /// ~50–200 ms and macOS's UserNotificationCenter queues them
    /// anyway — serializing with <c>await ShowAsync</c> inside the
    /// foreach used to mean 5 banners = 5×spawn serially while the TL
    /// had already rendered the new rows.
    /// </summary>
    private void WireDesktopNotifications(
    ...
    /// <summary>
    /// Build the per-row <see cref="TimelineItemContext"/> factory. Each
    /// row gets its own context instance; sharing a single one across
    /// rows would mean a fresh <c>OnFlash</c> bound to the wrong VM after
    /// hot-reload. The <paramref name="mainVmAccessor"/> indirection
    /// closes the chicken-and-egg between the factory needing the VM for
    /// <c>ShowFlash</c> and the VM being built after the factory.
    /// </summary>
    private Func<TimelineItemContext> BuildItemContextFactory(
        ...
        Ghuboon.App.Threading.ThreadingScheduler threading,
        Func<MainWindowViewModel?> mainVmAccessor)
    {
        ...
        return () => new TimelineItemContext(
            ...
            Log: _logger,
            Bookmarks: bookmarks,
            OnFlash: msg => mainVmAccessor()?.ShowFlash(msg),
            Threading: threading);
    }
